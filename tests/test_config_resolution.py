...
def test_resolve_config_entry_uses_matching_inner_model_name() -> None:
    ...
    key, params = resolve_config_entry(configs, "primitive_action_policy")

    assert key == "default_params"
    assert params["model_name"] == "primitive_action_policy"


def test_resolve_config_entry_prefers_explicit_top_level_key() -> None:
    configs = {
        "primitive_action_policy": {"model_name": "primitive_action_policy", "hidden_size": 128},
        "default_params": {"model_name": "primitive_action_policy", "hidden_size": 384},
    }

    key, params = resolve_config_entry(configs, "primitive_action_policy")

    assert key == "primitive_action_policy"
